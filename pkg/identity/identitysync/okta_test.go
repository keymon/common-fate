...
func TestOktaSync_Wildcard(t *testing.T) {
	...
	for _, gotUser := range users {
		switch gotUser.ID {
		case listOfUsers[0].Id:
			assert.ElementsMatch(t, gotUser.Groups, []string{listOfGroups[0].Id, listOfGroups[1].Id, listOfGroups[2].Id})
		case listOfUsers[1].Id:
			assert.ElementsMatch(t, gotUser.Groups, []string{listOfGroups[1].Id, listOfGroups[2].Id})
		case listOfUsers[2].Id:
			assert.ElementsMatch(t, gotUser.Groups, []string{listOfGroups[2].Id})
		}
	}
}
